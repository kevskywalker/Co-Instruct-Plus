...
from .data_utils import (
    ...
    get_image_info,
    get_image_info_with_square_padding,
...
from .path_utils import resolve_annotation_json, resolve_media_path
...
class SupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        if isinstance(data_path, str):
            if data_path.endswith(('.yaml', '.yml')):
                list_data_dict = self._load_yaml_datasets(data_path)
            else:
        ...
        self.image_pad_to_square = data_args.image_pad_to_square
    ...
    def _load_yaml_datasets(yaml_path):
        ...
        for ds in cfg.get("datasets", []):
            ...
            elif sampling.startswith("first:"):
                n = int(sampling.split(":")[1])
                all_data.extend(data[:n])
            elif sampling.startswith("end:"):
                n = int(sampling.split(":")[1])
                all_data.extend(data[-n:])
            elif sampling.startswith("random:"):
                import random
                n = int(sampling.split(":")[1])
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources:
            ...
            for image_file in image_files:
                ...
                image_input = get_image_info_with_square_padding(
                        image_file, 
                    ...
                    pad_to_square=self.image_pad_to_square,
                    )
            ...
            for video_file in video_files:
                if not video_file.startswith("http"):
                    video_file = resolve_media_path(video_file, video_folder)
