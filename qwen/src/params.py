...
class TrainingArguments(HFTrainingArguments):
    ...
    # DeQA-specific arguments
    dataset_type: str = field(default="pair", metadata={"help": "Dataset type: 'single' or 'pair'."})
    data_paths: List[str] = field(default_factory=list, metadata={"help": "Paths to training data JSON files."})
    data_weights: List[int] = field(default_factory=list, metadata={"help": "Weights for each training data file."})
    prebuilt_pairs: bool = field(default=False, metadata={"help": "Use prebuilt image pairs instead of online pairing."})
    level_prefix: str = field(default="", metadata={"help": "Prefix text before quality level token, e.g. 'The quality of the image is'."})
    level_names: List[str] = field(default_factory=list, metadata={"help": "Quality level names, e.g. ['excellent', 'good', 'fair', 'poor', 'bad']."})
    weight_desp: float = field(default=1.0, metadata={"help": "Weight for description loss."})
    weight_rank: float = field(default=1.0, metadata={"help": "Weight for ranking loss."})
    weight_softkl: float = field(default=1.0, metadata={"help": "Weight for soft KL loss."})
    weight_next_token: float = field(default=1.0, metadata={"help": "Weight for next token prediction loss."})
    weight_in_level: Optional[float] = field(default=None, metadata={"help": "Weight for in-level loss."})
    softkl_loss: bool = field(default=False, metadata={"help": "Use soft KL divergence loss on level tokens."})
    continuous_rating_loss: bool = field(default=True, metadata={"help": "Use continuous rating loss; else binary rating loss."})
    binary_rating_loss: str = field(default="fidelity", metadata={"help": "Binary rating loss type: 'bce' or 'fidelity'."})
    closeset_rating_loss: bool = field(default=False, metadata={"help": "Use closed-set softmax for quality level logits."})
    use_fix_std: bool = field(default=True, metadata={"help": "Use fixed std (=1) in rating loss."})
...
class DataArguments:
    ...
    image_pad_to_square: bool = field(default=False)
